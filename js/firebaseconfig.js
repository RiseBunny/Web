/*! RiseBunny Firebase Config + Bakım/Banner Motoru (v6) */
...
if (/admin\.html(\?|$)/.test(location.pathname)) return;   // 🔑 admin paneli ASLA etkilenmez
try {
var ADMIN_UID = window.ADMIN_UID;
var FORCE = /[?&]mnt=1/.test(location.search);
console.log('[RB] çekirdek motor yüklendi ✓');
...
/* Önce sayfanın kendi SDK'sını bekle (çift yükleme = çakışma olmasın), yoksa kendin yükle */
function waitSDK(cb, n) {
  if (window.firebase && window.firebase.firestore) return cb();
  if ((n || 0) > 25) return loadSDK('https://www.gstatic.com/firebasejs/10.12.2/firebase-app-compat.js')
    ...
    .then(cb);
  setTimeout(function () { waitSDK(cb, (n || 0) + 1); }, 100);
}
